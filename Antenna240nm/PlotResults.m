...
%
%    filename: /Users/felix/Documents/MATLAB/ManuscriptReviewerReply/GithubUpload/240nmAntennaAnalysis/ConnectedTriangle_PHZ051G02_X148G55H247p6_largerange_normalE.csv
%
% Auto-generated by MATLAB on 05-Jan-2021 19:38:18
...
opts.VariableNames = ["lam0m", "freqHz", "absnxExnyEynzEzVm", "argnxExnyEynzEzrad"];
opts.VariableTypes = ["double", "double", "double", "double"];
opts.ExtraColumnsRule = "ignore";
opts.EmptyLineRule = "read";

% Import the data
tbl = readtable("/Users/felix/Documents/MATLAB/ManuscriptReviewerReply/GithubUpload/240nmAntennaAnalysis/ConnectedTriangle_PHZ051G02_X148G55H247p6_largerange_normalE.csv", opts);

%% Convert to output type
lam0m = tbl.lam0m;
freqHz = tbl.freqHz;
absnxExnyEynzEzVm = tbl.absnxExnyEynzEzVm;
argnxExnyEynzEzrad = tbl.argnxExnyEynzEzrad;
...
clear opts tbl
  ...
  fig7=figure;
  set(gcf,'PaperUnits','centimeters','PaperSize',[7,5],'PaperPosition',[0 0 7 5]);
  yyaxis left
  plot(pulse4.frqz.*1e-15,abs(pulse4.spectrum)./max(abs(pulse4.spectrum)))
  ylim([0,1.1])
  ylabel('Amplitude')
  yyaxis right
  plot(freqHz.*1e-15,absnxExnyEynzEzVm)
  xlim([0.15,0.35])
  ylabel('Enhancement')
  xlabel('Frequency (PHz)')
  
print(fig7,strcat(dir,'/','Fig2b'),'-dpng','-r1200')
print(fig7,strcat(dir,'/','Fig2b'),'-depsc')

 %%
  phaseFitAntenna = linearPhaseFit(freqHz,unwrap(argnxExnyEynzEzrad),150e12,350e12)
  figure;
  plot(phaseFitAntenna,freqHz,unwrap(argnxExnyEynzEzrad))
  ...
  ylabel('Amplitude')
  yyaxis right
  plot(freqHz.*1e-15,unwrap(argnxExnyEynzEzrad)-phaseFitAntenna(freqHz))
  xlim([0.15,0.35])
  ylim([-pi,pi])
  ylabel('Phase')
  xlabel('Frequency (PHz)')
  
print(fig8,strcat(dir,'/','Fig2bphase'),'-dpng','-r1200')
print(fig8,strcat(dir,'/','Fig2bphase'),'-depsc')

%% Deconvolution of Electric Field
tPlasmon = data2.timeDelayResampled;
dtPl=tPlasmon(2)-tPlasmon(1);
LPl=length(tPlasmon);
frqzPl=[(0:LPl/2-1),-(LPl/2):-1]./(dtPl.*LPl)
fePhasedata=unwrap(argnxExnyEynzEzrad);
figure;
plot(freqHz,fePhasedata)

%%
indexPeak = find(freqHz==178447891666666.7);
fePhasedata(indexPeak)= (fePhasedata(indexPeak-1)+fePhasedata(indexPeak+1))./2;
figure;
plot(freqHz,fePhasedata)
%%
feInterp = interp1(freqHz,absnxExnyEynzEzVm,frqzPl,'linear',1);
fephaseInterp = interp1(freqHz,fePhasedata,frqzPl,'linear',0);
cmplxFE=feInterp.*exp(1i.*fephaseInterp);
impulseResponse=ifftshift(ifft(cmplxFE));

indexPeak2 = 5094;
impulseResponse(indexPeak2)= (impulseResponse(indexPeak2-1)+impulseResponse(indexPeak2+1))./2;
figure;
plot((real(impulseResponse)))
%%
convData=data2.dataAvg./max(data2.dataAvg);
convFD=fft(real(convData));
irFD=fft(real(impulseResponse)');
decon= ifft( convFD./irFD);

%%
[deconTest,~]=deconv(real(convData),real(impulseResponse)');
figure;
%plot(deconTest)
%%
...
%title(variableName);
%lgd=legend('0','\pi','Location','northwest');
legend('Deconvolved Data','Incident pulse');
%title(lgd,'CEP=');
set(ax,'FontSize',7);
box on
ax.YMinorTick='on';
ax.XMinorTick='on';
print(strcat(dir,'/','DeconvPlot'),'-dpng','-r1200')
print(strcat(dir,'/','DeconvPlot'),'-depsc')
